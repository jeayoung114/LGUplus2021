...
class DCNV2_implicit(torch.nn.Module):
    def __init__(self, train_data, train_label, valid_data, valid_label, field_dims, embed_dim, num_layers, mlp_dims, dropout,
                 num_epochs, early_stop_trial, learning_rate, reg_lambda, batch_size, device, num_experts, use_stacking=False):
        super().__init__()
    ...
    def build_graph(self):
        ...
        self.cn = CrossNetworkMMoE(self.embed_output_dim, self.num_layers, num_experts=self.num_experts)
        if self.use_stacking:
            output_layer=True
        else:
            output_layer=False
        self.mlp = MultiLayerPerceptron(self.embed_output_dim, self.mlp_dims, self.dropout, output_layer=output_layer)
    ...
    def forward(self, x):
        ...
        if self.use_stacking:
            x_l = self.cn(embed_x)
            h_l = self.mlp(x_l)
            output = torch.sigmoid(h_l.squeeze(1))
