...
def load_data_kg(data_name):
    data_loader = DataLoader_kg('music')
    kg = data_loader.load_kg()
    df_dataset = data_loader.load_dataset()
    num_user, num_entity, num_relation = data_loader.get_num()

    x_train, x_test, y_train, y_test = train_test_split(df_dataset, df_dataset['label'], test_size = 0.2, shuffle=False, random_state=12345)

    x_train, x_val, y_train, y_val = train_test_split(x_train, y_train, test_size = 0.2, shuffle=False, random_state=12345)

    return x_train, y_train, x_val, y_val, x_test, y_test, num_user, num_entity, num_relation, kg
